...
int main(int argc, char *argv[])
{
  ...
  gsl_vector vector_view;
  int i,n;
  long it,iters;
  double state_p,obs_p,state_p_true,obs_p_true,sumsq,sumsq2;
  if (argc != 2) {
    printf("Usage: %s <iters>\n",argv[0]);
    exit(1);
  }
  iters=atoi(argv[1]);
  n=200;           /* number of time series observations */
  ...
  gsl_vector_memcpy(data,&vector_view);
  gdag_free(d);
  /* Prior sampled */
  ...
  for (it=1;it<=iters;it++) {
    ...
    obs_p=gdag_ran_gamma(0.1+0.5*n,0.1+0.5*sumsq);
    gsl_vector_memcpy(scratch,s);
    gdag_vector_diff(scratch);
    gsl_blas_ddot(scratch,scratch,&sumsq);
    state_p=gdag_ran_gamma(0.1+0.5*(n-1),0.1+0.5*sumsq);
