...
int main(int argc,char *argv[])
{
  ...
  for (it=1;it<=iters;it++) {
    ...
    gsl_blas_ddot(&(vec_view.vector),&(vec_view.vector),&sumsq);
    tau_alpha=gdag_ran_gamma(rng,a_alpha+0.5*p,b_alpha+0.5*sumsq);
    /* update tau_beta */
    vec_view=gsl_vector_subvector(s,p+1,q);
    gsl_blas_ddot(&(vec_view.vector),&(vec_view.vector),&sumsq);
    tau_beta=gdag_ran_gamma(rng,a_beta+0.5*q,b_beta+0.5*sumsq);
    /* update tau_gamma */
    vec_view=gsl_vector_subvector(s,p+q+1,p*q);
    gsl_blas_ddot(&(vec_view.vector),&(vec_view.vector),&sumsq);
    tau_gamma=gdag_ran_gamma(rng,a_gamma+0.5*p*q,b_gamma+0.5*sumsq);
    /* update tau_eps */
    sumsq=0.0;
    for (i=0;i<p;i++) {
      for (j=0;j<q;j++) {
    ...
    tau_eps=gdag_ran_gamma(rng,a_eps+0.5*numrs,b_eps+0.5*sumsq);
    /* output iteration */
